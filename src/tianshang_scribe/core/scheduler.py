...
Schedules are stored in a :class:`~tianshang_scribe.utils.store.ScheduleStore` (SQLite).
...
(see :mod:`tianshang_scribe.cli.main`).
"""
...
from tianshang_scribe.utils.store import Schedule, ScheduleStore, now_iso
...
def build_command(args: list[str]) -> list[str]:
    ...
    return [sys.executable, '-m', 'tianshang_scribe.cli.main', *args]
