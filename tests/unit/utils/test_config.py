...
class TestSettingsEnvOverrides:
    def test_auth_token_env(self, monkeypatch) -> None:
        monkeypatch.setenv('TIANSHANG_SCRIBE_AUTH_TOKEN', 'secret')
    ...
    def test_both_token_sources_merged_deduplicated(self, monkeypatch) -> None:
        monkeypatch.setenv('TIANSHANG_SCRIBE_AUTH_TOKEN', 'primary')
        monkeypatch.setenv('TIANSHANG_SCRIBE_API_KEYS', 'primary, extra')
        assert Settings().bearer_tokens() == ['primary', 'extra']

    def test_host_port_env(self, monkeypatch) -> None:
        monkeypatch.setenv('TIANSHANG_SCRIBE_HOST', '192.168.1.5')
        monkeypatch.setenv('TIANSHANG_SCRIBE_PORT', '9000')
        settings = Settings()
        assert settings.host == '192.168.1.5'
        assert settings.port == 9000

    def test_cors_origins_env(self, monkeypatch) -> None:
        monkeypatch.setenv('TIANSHANG_SCRIBE_CORS_ORIGINS', 'https://a.dev, https://b.dev')
        assert Settings().cors_origin_list() == ['https://a.dev', 'https://b.dev']

    def test_rate_limit_env(self, monkeypatch) -> None:
        monkeypatch.setenv('TIANSHANG_SCRIBE_RATE_LIMIT_MAX', '50')
        monkeypatch.setenv('TIANSHANG_SCRIBE_RATE_LIMIT_WINDOW', '30')
        settings = Settings()
        assert settings.rate_limit_max == 50
        assert settings.rate_limit_window == 30

    def test_transport_env(self, monkeypatch) -> None:
        monkeypatch.setenv('TIANSHANG_SCRIBE_TRANSPORT', 'sse')
        assert Settings().transport == 'sse'

    def test_unrelated_env_ignored(self, monkeypatch) -> None:
        monkeypatch.setenv('TIANSHANG_SCRIBE_SOMETHING_ELSE', 'x')
        assert Settings().auth_token is None


class TestSettingsValidation:
    def test_invalid_transport_rejected(self, monkeypatch) -> None:
        monkeypatch.setenv('TIANSHANG_SCRIBE_TRANSPORT', 'carrier-pigeon')
        with pytest.raises(ValidationError):
            Settings()

    def test_invalid_port_rejected(self, monkeypatch) -> None:
        monkeypatch.setenv('TIANSHANG_SCRIBE_PORT', 'not-a-number')
        with pytest.raises(ValidationError):
            Settings()
