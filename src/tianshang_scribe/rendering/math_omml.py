...
_MATHTYPE_TEXT_RE = re.compile(r'\\(text|mathrm|mathsf|mathtt|mathit)\{([^{}]*)\}')
...
def _extract_one_arg(s: str, start: int) -> tuple[int, int, int] | None:
...
def _extract_sqrt_args(s: str, start: int) -> tuple[tuple[int, int], tuple[int, int, int]] | None:
...
def _handle_frac(ctx: ParserContext, _cmd: str, cmd_end: int) -> list[Token] | None:
...
def _handle_sqrt(ctx: ParserContext, _cmd: str, cmd_end: int) -> list[Token] | None:
    args = _extract_sqrt_args(ctx.latex, cmd_end)
    if args:
        (ds, de), (cs, ce, nx) = args
        degree_tokens: list[Token] | None = None
        if ds < de:
            degree_tokens = parse_expression(ParserContext(ctx.latex, ds, de))
...
def _handle_delim(ctx: ParserContext, cmd: str, cmd_end: int) -> list[Token] | None:
...
def _handle_nary(ctx: ParserContext, cmd: str, cmd_end: int) -> list[Token] | None:
...
def _handle_operator_limits(ctx: ParserContext, cmd: str, cmd_end: int) -> list[Token] | None:
    limits = _extract_limits(ctx.latex, cmd_end)
...
def _handle_simple_operator(ctx: ParserContext, cmd: str, cmd_end: int) -> list[Token]:
    ctx.pos = cmd_end
    return [OperatorToken(op=cmd)]


def _handle_accent(ctx: ParserContext, cmd: str, cmd_end: int) -> list[Token] | None:
...
def _handle_mbsp(_ctx: ParserContext, _cmd: str, cmd_end: int) -> list[Token]:
    _ctx.pos = cmd_end
...
def _handle_greek(ctx: ParserContext, cmd: str, cmd_end: int) -> list[Token]:
    ctx.pos = cmd_end
    return [TextToken(GREEK_MAP[cmd], cmd[0].isupper())]


def _handle_symbol(ctx: ParserContext, cmd: str, cmd_end: int) -> list[Token]:
    ctx.pos = cmd_end
    return [TextToken(SYMBOL_MAP[cmd])]


def _handle_styled(ctx: ParserContext, cmd: str, cmd_end: int) -> list[Token] | None:
    span = _extract_one_arg(ctx.latex, cmd_end)
    if span is not None:
        cs, ce, nx = span
        content = parse_expression(ParserContext(ctx.latex, cs, ce))
        ctx.pos = nx
        return [StyledToken(style=_STYLE_CMD_MAP.get(cmd, 'normal'), content=content)]
    ctx.pos = cmd_end
    return None
...
_TOP_LEVEL_EXCLUDE = frozenset(('\\', '^', '_', ' ', '(', ')', '[', ']', '{', '}'))
_BODY_EXCLUDE = frozenset(('\\', '^', '_', ' '))


def _collect_text_run(ctx: ParserContext, exclude: frozenset[str]) -> list[Token]:
    tokens: list[Token] = []
...
def _collect_body_tokens(latex: str, start: int) -> tuple[list[Token], int]:
    ...
    while pos < length:
        ...
        if ch == '\\':
            ...
            if pos < length and latex[pos] == '{':
                ...
                if span is not None:
                    cs, ce, nx = span
                    content = parse_expression(ParserContext(latex, cs, ce))
                    pos = nx
                ...
                ch2 = latex[pos]
                pos += 1
                _attach_script(tokens, kind, [TextToken(ch2, ch2.isdigit())])
            continue
        elif ch == ' ':
            pos += 1
            continue
...
def _build_styled_elements(content: list[Token], style: str) -> list[Any]:
    norm = style != 'italic'
