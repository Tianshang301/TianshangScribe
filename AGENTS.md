...
└── tianshang_scribe/         # 可 import 包（tianshang_scribe.*，对标 TianshangCAD 的 tianshangcad）
    ...
    │   ├── tools/          # 14 个 Agent 工具实现（7 统一 + 5 专用 + 2 对比/检查）
...
| v0.8.1 | 2026-08 | **代码债务快修（PLAN.md P1-010~013）**：`_parse.py` 条件格式/数据验证 spec 解析改 `split(':', 2)`（含 `:` 的公式如时间字面量不再被切碎，超段保留于 `formula2` 不再静默丢弃，P1-012）；PPT `add_textbox` 的 `top` 改可选 `None`——省略时经 `_text_cursors` 游标自动垂直堆叠不重叠、显式 top 行为不变，`PptTextBlock.top` 默认同步改 `None`（P1-011）；`write_cell` 新增三态 `is_formula`（`true`=按公式存储且须以 `=` 开头 / `false`=强制文本单元格 `data_type='s'` / 缺省=自动），消除 `=` 前缀纯文本写入歧义，统一与专用 schema 同步（P1-013）；registry 将 `edit_office_document` 描述标注 Legacy 并引导至 `edit_excel_workbook`/`edit_presentation`，能力广告迁移至专用工具描述（P1-010 方案 A）；`SERVER_VERSION` 与包版本同步至 0.8.1。新专用工具（compare_excel_workbooks/extract_presentation_data）留待 0.9.0，对比范围定为值+公式优先 |
| v0.9.0 | 2026-08 | **MCP 能力补全与 L2 schema 全量接线（PLAN.md 附录 B，D-7 能力冻结）**：新增第 13/14 个工具——`compare_excel_workbooks`（工作表级增删改名映射 + 单元格级 diff，data/formula/full 三模式；formula 模式经实测 openpyxl read_only 流式读取可返回公式串，O(1) 内存 + 哈希预滤，偏离 D-5 原判并记录）与 `extract_presentation_data`（outline/structure 外新增 notes 全文与 master_info 母版版式清单模式）；`analyze_excel_data` 新增 pivot_suggestion 模式（类型推断建议行列值聚合）；PPT 引擎新增 `apply_theme` 内置主题（office/dark，直接重写 theme1.xml clrScheme/fontScheme）与 `add_movie`/`add_audio` 点击播放媒体插入、母版级页脚/页码/日期占位 `set_master_options`；Excel 引擎新增行/列分组 `group_rows`/`group_columns`/`ungroup`（outline_level/hidden）、`set_tab_color`、`set_print_area`、精简版 `set_page_setup`；L2 接线遵循 D-7 冻结——legacy `EditOperation` 仅收编 group_rows/group_columns/ungroup/add_media 四个核心 action，边缘认知（tab color/print area/page setup/theme/master options）仅经专用工具 `ExcelEditOp`(17 action)/`PptEditOp`(13 action) 直达引擎，公共 `run_edit_session` 壳保持单会话有序分发；横切：错误响应支持可选 `field`/`documentation_url`，错误码细分 EXCEL_INVALID_CELL_REF(1007)/EXCEL_INVALID_RANGE(1008)/PPT_INVALID_SLIDE_INDEX(1009)/EXCEL_SHEET_NOT_FOUND(1010)，新增 `_dryrun.py` 预检模块（cell/range/sheet/slide-index 校验 + 影响单元格估算，明确不做公式语法校验），edit/create 的 dry_run 返回逐操作 `validations`/`all_valid`/`estimated_impacted_cells`；stdio/SSE 冒烟断言同步 14；`SERVER_VERSION` 与包版本同步至 0.9.0 |