"""Agent simulation: test all 5 MCP tools with realistic workflows."""
import json
import subprocess
import sys
import tempfile
...
SERVER = [sys.executable, '-m', 'mcp.server']
...
def call(method: str, params: dict = None, msg_id: int = 1) -> dict:
    request = json.dumps({
    ...
    if not raw:
        raise RuntimeError(f'No output. stderr: {proc.stderr[:200]}')
...
result = tool('edit_office_document', {
...
result = tool('extract_document_data', {
    'input_path': str(OUT / 'report.docx'),
    'mode': 'structure',
})
