...
from tianshang_scribe.cli.global_opts import (
    check_overwrite,
...
from tianshang_scribe.core.document import (
    DocumentType,
    create_document,
    ...
    def _process_one(current_input: str | None) -> None:
        ...
        try:
            ...
            if to_image and hasattr(engine, 'to_images'):
                ...
                from tianshang_scribe.transform.pdf import word_to_markdown
                ...
                if hasattr(engine, 'export_html'):
                    ...
                    from tianshang_scribe.transform.pdf import word_to_html
...
def _convert_source(input_path: str, output_path: str, to_pdf: bool = False) -> None:
    ...
    if suffix == '.json':
        ...
        from tianshang_scribe.transform.reverse import html_to_word, markdown_to_word
    ...
    if to_pdf:
        from tianshang_scribe.transform.pdf import word_to_pdf
...
def _open_schedule_store(db: str | None) -> Any:
    ...
    from tianshang_scribe.utils.store import ScheduleStore
...
def _handle_schedule(
    *,
    ...
    try:
        if add_spec:
            from tianshang_scribe.core.scheduler import parse_command
            from tianshang_scribe.utils.store import Schedule
        ...
        if run_name:
            from tianshang_scribe.core.scheduler import TaskScheduler
...
def _handle_run_script(path: str) -> None:
    ...
    from tianshang_scribe.core.script_runner import ScriptRunner
