...
> 基于当前实现（纯 `asyncio` JSON-RPC 2.0、5 tools、stdio/SSE 双模式）的系统性改进方案。
...
| 机制 | 实现方式 | 配置 |
|------|---------|------|
| Bearer Token 认证 | `Authorization: Bearer <token>` 头验证 | `--auth-token $SCRIBE_TOKEN` |
| CORS 白名单 | 替换当前通配 `*` 为可配置 origin | `--cors-origins "https://coze.com,https://dify.ai"` |
| 请求签名 | HMAC-SHA256 对 `POST /message` body 签名 | `--sign-secret $SCRIBE_SECRET` |
...
| Metric | 类型 | 说明 |
|--------|------|------|
| `scribe_tools_total{tool,format}` | Counter | 各工具调用次数 |
| `scribe_duration_seconds{tool}` | Histogram | 操作耗时分布 |
| `scribe_errors_total{code}` | Counter | 错误码分布 |
| `scribe_file_size_bytes{format}` | Summary | 处理的文件大小 |
| `scribe_active_sessions` | Gauge | 当前活跃 session 数 |
...
"format": {
    ...
    "description": (
        ...
        "- 'xlsx'：Excel 工作簿（电子表格、数据分析）\n"
        "- 'pptx'：PowerPoint 演示文稿（幻灯片、发布会）"
    ),
    "examples": ["docx", "xlsx"]
}
```
